...
def build_grid(paths):
    ...
    grid = [["." for _ in range(SIZE_X)] for __ in range(SIZE_Y)]
    ...
    SAND_POSITION[X] = SAND_POSITION[X] - top_left[X]
...
def draw_sand(grid, SAND_POSITION):
    ...
    return grid, new_position != MOVE_OUT
...
i = 0
while hasChanged:
    grid, hasChanged = draw_sand(grid, SAND_POSITION)
    i += 1
